...
void Load_Problem( struct Problem_Info *pi, struct Aquifer_Data *ad, struct Well_Data *wd, struct Point_Data *pd )
{
	...
	for( i = 0; i < (*wd).nW; i++ )
	{
		...
		printf( "%s x=%g y=%g rw=%g nQ=%i ", (*wd).id[i], (*wd).x[i], (*wd).y[i], (*wd).r[i], (*wd).nQ[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).h[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).m[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).k[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).S[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).B[i] );
		printf( "h0=%g m=%g k=%g S=%g B=%g\n", (*wd).h[i], (*wd).m[i], pow(10,(*wd).k[i]), pow(10,(*wd).S[i]), pow(10,(*wd).B[i]) );
...
double Theis_unc( double x, double y, double time, double h, double *k, double *S, struct Well_Data *wd )
{
	double dx, dy, r, h2, sub1, sub2, sub3, sub4, s, ss;
	int i, n, nR;
	...
	for( i = 0; i < (*wd).nW; i++ )
	{
		...
		if( sub4 > 0 )
		{
			ss = h - sqrt( h2 - (*wd).Q[i][0] * Ei( sub3 / sub4 ) / sub1 ); /* initial drawdown */
			(*wd).sw[i] += ss;
			s += ss;
		}
		nR = (*wd).nQ[i];
		for( n = 1; n < nR && (*wd).t[i][n] < time; n++ )
		{
			...
			if( sub4 > 0 )
			{
				ss = h - sqrt( h2 - ( (*wd).Q[i][n] - (*wd).Q[i][n - 1] ) * Ei( sub3 / sub4 ) / sub1 );
				(*wd).sw[i] += ss;
				s += ss;
			}
		}
...
double Hantush( double x, double y, double time, double h, double *k, double *S, struct Well_Data *wd )
{
	double dx, dy, r, T, sub1, sub2, sub3, sub4, sub5, s, ss;
	int i, n, nR;
	...
	for( i = 0; i < (*wd).nW; i++ )
	{
		...
		if( sub5 > 0 )
		{
			ss = (*wd).Q[i][0] * W( sub3 / sub5, sub4 ) / sub1; /* initial drawdown */
			(*wd).sw[i] += ss;
			s += ss;
		}
		nR = (*wd).nQ[i];
		for( n = 1; n < nR && (*wd).t[i][n] < time; n++ )
		{
			...
			if( sub5 > 0 )
			{
				ss = ( (*wd).Q[i][n] - (*wd).Q[i][n - 1] ) * W( sub3 / sub5, sub4 ) / sub1;
				(*wd).sw[i] += ss;
				s += ss;
			}
		}
...
double Hantush_unc( double x, double y, double time, double h, double *k, double *S, struct Well_Data *wd )
{
	double dx, dy, r, h2, sub1, sub2, sub3, sub4, sub5, s, ss;
	int i, n, nR;
	...
	for( i = 0; i < (*wd).nW; i++ )
	{
		...
		if( sub5 > 0 )
		{
			ss = h - sqrt( h2 - (*wd).Q[i][0] * W( ( sub3 / sub5 ), sub4 ) / sub1 ); /* initial drawdown */
			(*wd).sw[i] += ss;
			s += ss;
		}
		nR = (*wd).nQ[i];
		for( n = 1; n < nR && (*wd).t[i][n] < time; n++ )
		{
			...
			if( sub5 > 0 )
			{
				ss = h - sqrt( h2 - ( (*wd).Q[i][n] - (*wd).Q[i][n - 1] ) * W( ( sub3 / sub5 ), sub4 ) / sub1 );
				(*wd).sw[i] += ss;
				s += ss;
			}
		}
