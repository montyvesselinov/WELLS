...
double Hantush( double x, double y, double time, struct Aquifer_Data ad, struct Well_Data *wd );
double Hantush_unc( double x, double y, double time, struct Aquifer_Data ad, struct Well_Data *wd );
double Ei( double u );
...
void Calc_Wells( struct Aquifer_Data ad, struct Well_Data *wd )
{
	...
	double x, y, xc, yc, ca, sa,
		  (*fp)( double x, double y, double time, struct Aquifer_Data ad, struct Well_Data *wd );
...
void Calc_Save_Grid( double time, struct Aquifer_Data ad, struct Well_Data *wd, struct Grid_Data *gd, char *fileName )
{
	...
	int   i, j, code1, code2, code3;
	char  buf[30];
	double s, xg, yg, dx, dy, x, y, xc, yc, ca, sa,
		  (*fp)( double x, double y, double time, struct Aquifer_Data ad, struct Well_Data *wd );
...
double Hantush_unc( double x, double y, double time, struct Aquifer_Data ad, struct Well_Data *wd )
{
	...
	for( i = 0; i < (*wd).nW; i++ )
	{
		...
		sub5 = time - (*wd).t[i][0];
		if( sub5 > 0 )
			s += ad.h - sqrt( h2 - (*wd).Q[i][0] * W( ( sub3 / sub5 ), sub4 ) / sub1 );
		nR = (*wd).nQ[i];
		for( n = 1; n < nR && (*wd).t[i][n] < time; n++ )
		{
			sub5 = time - (*wd).t[i][n];
			if( sub5 > 0 )
				s += ad.h - sqrt( h2 - ( (*wd).Q[i][n] - (*wd).Q[i][n - 1] ) * W( ( sub3 / sub5 ), sub4 ) / sub1 );
		}
	}
	return( s );
}
