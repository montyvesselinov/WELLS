...
double Theis( double x, double y, double time, double h, double *k, double *kc1, double *S, struct Well_Data *wd );
/*
...
void Calc_Points( struct Aquifer_Data ad, struct Well_Data *wd, struct Point_Data *pd )
{
	...
	double x, y, xc, yc, ca, sa, h, k, S,
		  (*fp)( double x, double y, double time, double h, double *k, double *kc1, double *S, struct Well_Data *wd );
...
//	}
	...
	for( i = 0; i < (*pd).nP; i++ )
	{
		for( j = 0; j < (*pd).nT[i]; j++ )
		{
			(*pd).s[i][j] = fp( (*pd).x[i], (*pd).y[i], (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], wd );
			if( ad.co_x_axis != NO_BOUNDARY || ad.co_y_axis != NO_BOUNDARY )
			{
				...
				if( code1 )
				{
					xc =x * ca - -y * sa + ad.origin_x;
					yc = -y * ca +x * sa + ad.origin_y;
					(*pd).s[i][j] += code1 * fp( xc, yc, (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], wd );
				}
				if( code2 )
				{
					xc = -x * ca -y * sa + ad.origin_x;
					yc =y * ca + -x * sa + ad.origin_y;
					(*pd).s[i][j] += code2 * fp( xc, yc, (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], wd );
				}
				if( code3 )
				{
					xc = -x * ca - -y * sa + ad.origin_x;
					yc = -y * ca + -x * sa + ad.origin_y;
					(*pd).s[i][j] += code3 * fp( xc, yc, (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], wd );
				}
			}
		}
	}
...
double Theis( double x, double y, double time, double h, double *k, double *kc1, double *S, struct Well_Data *wd )
{
	double dx, dy, r2, r2_well, T, sub, sub2, s, del_t, four_pi;
	int i, n, nR;
