...
double Theis( double x, double y, double time, double h, double *k, double *kc1, double *S, double *Sc1, struct Well_Data *wd );
/*
...
float **Matrix( int maxCols, int maxRows );
...
void Load_Problem( struct Problem_Info *pi, struct Aquifer_Data *ad, struct Well_Data *wd, struct Point_Data *pd )
{
	...
	if( ( (*wd).h = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
	if( ( (*wd).m = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
	if( ( (*wd).k = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
	if( ( (*wd).kc1 = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
	if( ( (*wd).S = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
	if( ( (*wd).Sc1 = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
	if( ( (*wd).B = (double *) malloc( (*wd).nW * sizeof( double ) ) ) == NULL ) {printf( "Not enough memory!\n" ); exit( 1 );};
...
/*		printf( "%s %f %f %f %i\n", (*wd).id[i], (*wd).x[i], (*wd).y[i], (*wd).r[i], (*wd).nQ[i] ); */
		...
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).m[i] );
		fscanf( infile, "%[^:]s", buf ); 	
	 	kn = fscanf( infile, ": %lf %lf\n", &(*wd).k[i], &(*wd).kc1[i] ); 
		if( kn < 2 )
		{
			printf( "Slope parameter required for time dependent transmissivity parameter!!!\n" );
			exit(0);
		}
		fscanf( infile, "%[^:]s", buf ); 
		Sn = fscanf( infile, ": %lf %lf\n", &(*wd).S[i], &(*wd).Sc1[i] );
		//printf("%d\n", Sn);
		if( Sn < 2 )
		{
			printf( "Slope parameter required for time dependent storativity parameter!!!\n" );
			exit(0);
		}
		//printf("%f %f\n", (*wd).S[i], (*wd).Sc1[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*wd).B[i] );
...
void Calc_Wells( struct Aquifer_Data ad, struct Well_Data *wd )
{
	...
	double x, y, xc, yc, ca, sa, h, k, kc1, kc2, S,
		  (*fp)( double x, double y, double time, double h, double *k, double *kc1, double *kc2, double *S, struct Well_Data *wd );
...
{
	int   i, j, code1, code2, code3;
	double x, y, xc, yc, ca, sa, h, k, S,
		  (*fp)( double x, double y, double time, double h, double *k, double *kc1, double *S, double *Sc1, struct Well_Data *wd );
...
//	}
	...
	for( i = 0; i < (*pd).nP; i++ )
	{
		for( j = 0; j < (*pd).nT[i]; j++ )
		{
			(*pd).s[i][j] = fp( (*pd).x[i], (*pd).y[i], (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], ad.Sc1obs[i], wd );
			if( ad.co_x_axis != NO_BOUNDARY || ad.co_y_axis != NO_BOUNDARY )
			{
				...
				if( code1 )
				{
					xc =x * ca - -y * sa + ad.origin_x;
					yc = -y * ca +x * sa + ad.origin_y;
					(*pd).s[i][j] += code1 * fp( xc, yc, (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], ad.Sc1obs[i], wd );
				}
				if( code2 )
				{
					xc = -x * ca -y * sa + ad.origin_x;
					yc =y * ca + -x * sa + ad.origin_y;
					(*pd).s[i][j] += code2 * fp( xc, yc, (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], ad.Sc1obs[i], wd );
				}
				if( code3 )
				{
					xc = -x * ca - -y * sa + ad.origin_x;
					yc = -y * ca + -x * sa + ad.origin_y;
					(*pd).s[i][j] += code3 * fp( xc, yc, (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.kc1obs[i], ad.Sobs[i], ad.Sc1obs[i], wd );
				}
			}
		}
	}
...
double Theis( double x, double y, double time, double h, double *k, double *kc1, double *S, double *Sc1, struct Well_Data *wd )
{
	double dx, dy, r2, r2_well, T, S_t,sub, sub2, s, del_t, four_pi;
	int i, n, nR;
...
//		sub1 = 12.5663706 * T; /* 4 * PI = 12.5663706 */
		four_pi = 12.5663706; /* 4 * PI = 12.5663706 */
//		sub2 = S[i] / ( 4.0 * T );
...
//		sub3 = r * sub2;
		...
		if( del_t > 0 )
		{
			T = (*wd).m[i] * k[i] * exp( kc1[i] / del_t );
			if( T > 1E300 ) T = 1E300; 
			S_t = S[i] * exp( Sc1[i] / del_t ); 
			if( S_t > 1E300 ) S_t = 1E300;
			sub = S_t * r2 / ( 4 * T * del_t );
			s += (*wd).Q[i][0] * Ei( sub ) / ( four_pi * T ) ; /* initial drawdown */
		}
		...
		for( n = 1; n < nR && (*wd).t[i][n] < time; n++ ) /* loop through all the pumping rates */
		{
			...
			if( del_t > 0 )
			{
				...
				if( S_t > 1E300 ) S_t = 1E300;
				sub = S_t * r2 / ( 4 * T * del_t );
//		printf( "%lf %lf %lf %lf %lf\n", k[i], kc1[i], T, sub, del_t );
				s += ( (*wd).Q[i][n] - (*wd).Q[i][n - 1] ) * Ei( sub ) / ( four_pi * T );
//				printf("%le %le %le %le %le %le %le\n", s, T, S_t, del_t, sub, (*wd).Q[i][n], (*wd).Q[i][n-1]);
			}
		}
	}
//	printf("%le %le %le\n", s, T, S_t);
...
double Theis_unc( double x, double y, double time, double h, double *k, double *S, struct Well_Data *wd )
{
	...
	for( i = 0; i < (*wd).nW; i++ )
	{
		...
		for( n = 1; n < nR && (*wd).t[i][n] < time; n++ )
		{
			sub4 = time - (*wd).t[i][n];
			if( sub4 > 0 )
				s += h - sqrt( h2 - ( (*wd).Q[i][n] - (*wd).Q[i][n - 1] ) * Ei( sub3 / sub4 ) / sub1 );
		}
	}
	return( s );
}
