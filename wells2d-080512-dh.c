...
void Load_Problem( struct Problem_Info *pi, struct Aquifer_Data *ad, struct Well_Data *wd, struct Point_Data *pd )
{
	...
	for( i = 0; i < (*pd).nP; i++ )
	{
		...
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*pd).h[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*pd).k[i] );
		(*pd).k[i] = pow( 10, (*pd).k[i] );
		fscanf( infile, "%[^:]s", buf ); fscanf( infile, ": %lf\n", &(*pd).S[i] );
		(*pd).S[i] = pow( 10, (*pd).S[i] );
		for( j = 0; j < (*pd).nT[i]; j++ )
			fscanf( infile, "%lf\n", &(*pd).t[i][j] );
	}
	fclose( infile );
	...
	for ( i = 0; i < (*pd).nP; i++ )
	{
		for ( j = 0; j < (*wd).nW; j++ )
		{
			(*ad).kobs[i][j] = 2/( 1/(*wd).k[j] + 1/(*pd).k[i] );
			printf( "%g %g %g\n", (*ad).kobs[i][j], (*wd).k[j], (*pd).k[i] );
			(*ad).Sobs[i][j] = pow( (*wd).S[j] * (*pd).S[i], 0.5 );
		}
	}
	for ( i = 0; i < (*wd).nW; i++ )
	{
		for ( j = 0; j < (*wd).nW; j++ )
		{
			(*ad).kprod[i][j] = 2/( 1/(*wd).k[i] + 1/(*wd).k[j] );
			(*ad).Sprod[i][j] = pow( (*wd).S[i] * (*wd).S[j], 0.5 );
		}
	}
...
void Calc_Points( struct Aquifer_Data ad, struct Well_Data *wd, struct Point_Data *pd )
{
	...
	for( i = 0; i < (*pd).nP; i++ )
	{
		for( j = 0; j < (*pd).nT[i]; j++ )
		{
			(*pd).s[i][j] = fp( (*pd).x[i], (*pd).y[i], (*pd).t[i][j], (*pd).h[i], ad.kobs[i], ad.Sobs[i], wd );
			if( ad.co_x_axis != NO_BOUNDARY || ad.co_y_axis != NO_BOUNDARY )
			{
